  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
      ...
      <ul className='hidden md:flex items-start gap-5 font-medium'>
        ...
        <MenuItem to="/doctors" onClick={() => setShowMenu(false)}>ALL CATEGORIES</MenuItem>
        <MenuItem to="/about" onClick={() => setShowMenu(false)}>ABOUT</MenuItem>
        <MenuItem to="/contact" onClick={() => setShowMenu(false)}>CONTACT</MenuItem>
        <MenuItem to='/funding' onClick={() => setShowMenu(false)}>FUNDING</MenuItem>
      ...
      <div className='flex items-center gap-4 relative'>
        ...
        <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-2 mt-5'>
            <MenuItem to='/' onClick={() => setShowMenu(false)}>HOME</MenuItem>
            <MenuItem to='/doctors' onClick={() => setShowMenu(false)}>ALL CATEGORIES</MenuItem>
            <MenuItem to='/about' onClick={() => setShowMenu(false)}>ABOUT US</MenuItem>
            <MenuItem to='/contact' onClick={() => setShowMenu(false)}>CONTACT US</MenuItem>
          </ul>
        </div>
      </div>
    </div>
